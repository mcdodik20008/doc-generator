...
internal object NkxUtil {
    ...
    fun hasAnyAnn(a: Set<String>, vararg keys: String) = keys.any { it.lowercase() in a }
    fun superContains(s: Set<String>, vararg keys: String) = keys.any { it.lowercase() in s }
    fun importsContain(imps: List<String>, vararg parts: String) =
        parts.any { p -> imps.any { it.contains(p.lowercase()) } }

    fun nameEnds(n: String, vararg suff: String) = suff.any { n.endsWith(it, ignoreCase = true) }
    fun nameContains(n: String, vararg subs: String) = subs.any { n.contains(it, ignoreCase = true) }
}