    ...
    private val nodeKindExtractors: List<NodeKindExtractor>,
        ...
        fun execute(cmd: DeclCmd) {
            when (cmd) {
                ...
                is UpsertTypeCmd -> {
                    ...
                    val kind = refineKindForType(cmd.baseKind, r, fileUnitByPath[r.filePath])
                ...
                is UpsertFieldCmd -> {
                    ...
                    upsertNode(
                        ...
                        kind = refineKindForField(NodeKind.FIELD, r, fileUnitByPath[r.filePath]),
                        name = r.name,
                        packageName = pkg,
                        parent = parent,
                        lang = Lang.kotlin,
                ...
                is UpsertFunctionCmd -> {
                    ...
                    val node = upsertNode(
                        fqn = fqn,
                        kind = refineKindForFunction(NodeKind.METHOD, r, fileUnitByPath[r.filePath]),
                        name = r.name,
                        packageName = pkgFqn,
                        parent = parent,
                        lang = Lang.kotlin,
        ...
        private fun upsertNode(
            ...
            return if (existing == null) {
                ...
                var changed = false
                fun <T> setIfChanged(curr: T, new: T, apply: (T) -> Unit) { if (curr != new) { apply(new); changed = true } }
        ...
        private fun refineKindForType(base: NodeKind, raw: RawType, fileUnit: RawFileUnit?): NodeKind {
            if (nodeKindExtractors.isEmpty()) return base
            val ctx = NodeKindContext(lang = Lang.kotlin, file = fileUnit, imports = fileUnit?.imports)
            return nodeKindExtractors.asSequence()
                .filter { it.supports(ctx.lang) }
                .mapNotNull { it.refineType(base, raw, ctx) }
                .firstOrNull() ?: base
        }
        private fun refineKindForFunction(base: NodeKind, raw: RawFunction, fileUnit: RawFileUnit?): NodeKind {
            if (nodeKindExtractors.isEmpty()) return base
            val ctx = NodeKindContext(lang = Lang.kotlin, file = fileUnit, imports = fileUnit?.imports)
            return nodeKindExtractors.asSequence()
                .filter { it.supports(ctx.lang) }
        ...
        private fun refineKindForField(base: NodeKind, raw: RawField, fileUnit: RawFileUnit?): NodeKind {
            if (nodeKindExtractors.isEmpty()) return base
            val ctx = NodeKindContext(lang = Lang.kotlin, file = fileUnit, imports = fileUnit?.imports)
            return nodeKindExtractors.asSequence()
                .filter { it.supports(ctx.lang) }
