...
class NodeDocGeneratorTest {
    ...
    fun `generate - успешно генерирует документ для метода`() {
        ...
        val result = generator.generate(node, "ru")
    ...
    @Test
    fun `generate - always proceeds to LLM (SQL handles readiness)`() {
        val app = Application(key = "app1", name = "App1")
        app.id = 1L
        val node = Node(application = app, fqn = "com.example.Method1", kind = NodeKind.METHOD, lang = Lang.kotlin)
        node.id = 100L
        ...
        verify(exactly = 1) { coder.generate(any(), any()) }
    }

    @Test
    fun `generate - uses fallback digest when digestClient returns blank`() {
        val app = Application(key = "app1", name = "App1")
        app.id = 1L
        val node = Node(application = app, fqn = "com.example.Method1", kind = NodeKind.METHOD, lang = Lang.kotlin)
        node.id = 100L
        ...
        every { contextBuilder.build(node, "ru") } returns buildResult
        every { promptRegistry.resolve(any()) } returns prompt
        every { coder.generate(any(), any()) } returns "tech doc content here"
        every { talker.rewrite(any()) } returns "public doc"
        every { digestClient.generate(any(), any(), any(), any()) } returns ""

        val result = generator.generate(node, "ru")

        // Fallback: первые 500 символов docTech
        assertThat(result.docDigest).isEqualTo("tech doc content here")
    }

    @Test
    fun `generate - fallback digest uses no-content when docTech is also blank`() {
        val app = Application(key = "app1", name = "App1")
        app.id = 1L
        val node = Node(application = app, fqn = "com.example.Method1", kind = NodeKind.METHOD, lang = Lang.kotlin)
        node.id = 100L
        ...
        every { contextBuilder.build(node, "ru") } returns buildResult
        every { promptRegistry.resolve(any()) } returns prompt
        every { coder.generate(any(), any()) } returns ""
        every { talker.rewrite(any()) } returns "public doc"
        every { digestClient.generate(any(), any(), any(), any()) } returns ""

        val result = generator.generate(node, "ru")

        assertThat(result.docDigest).isEqualTo("no-content")
    }
