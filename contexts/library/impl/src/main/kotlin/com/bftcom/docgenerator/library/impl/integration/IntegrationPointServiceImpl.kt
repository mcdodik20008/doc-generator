...
class IntegrationPointServiceImpl(
    ...
    override fun findParentClients(libraryId: Long): List<LibraryNode> {
        return libraryNodeRepo.findParentClientsByLibraryId(libraryId)
    }
    
    override fun findMethodsByUrl(url: String, libraryId: Long?): List<LibraryNode> {
        // Обертываем URL в JSON строку для поиска в массиве
        val urlJson = "\"$url\""
        return libraryNodeRepo.findMethodsByUrl(urlJson, libraryId)
    }
    
    override fun findMethodsByKafkaTopic(topic: String, libraryId: Long?): List<LibraryNode> {
        // Обертываем topic в JSON строку для поиска в массиве
        val topicJson = "\"$topic\""
        return libraryNodeRepo.findMethodsByKafkaTopic(topicJson, libraryId)
    }
    
    override fun findMethodsByCamelUri(uri: String, libraryId: Long?): List<LibraryNode> {
