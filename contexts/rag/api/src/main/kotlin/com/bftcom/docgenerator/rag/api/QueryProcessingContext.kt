...
class QueryProcessingContext(
     ...
     */
    fun <T> getMetadata(key: QueryMetadataKeys): T? = metadata[key.key] as? T

    /**
     * Безопасная установка метаданных через enum ключ
     */
    fun setMetadata(
        key: QueryMetadataKeys,
        value: Any,
     ...
     */
    fun hasMetadata(key: QueryMetadataKeys): Boolean = metadata.containsKey(key.key)
