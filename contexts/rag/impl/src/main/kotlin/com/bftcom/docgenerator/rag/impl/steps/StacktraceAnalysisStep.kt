...
class StacktraceAnalysisStep(
    ...
    companion object {
        private val EXPANSION_EDGE_KINDS = setOf(
            EdgeKind.CALLS_CODE, EdgeKind.CALLS_HTTP, EdgeKind.READS,
            EdgeKind.WRITES, EdgeKind.DEPENDS_ON,
        )
    }

    override fun execute(context: QueryProcessingContext): StepResult {
        val appFrames = context.getMetadata<List<*>>(QueryMetadataKeys.STACKTRACE_APP_FRAMES)
        ...
        val rootCauseNode = frameMappings.find {
        ...
        val analysisText = buildAnalysisText(
            exceptionType, exceptionMessage, frameMappings, rootCauseNode, graphRelationsText,
        )
        ...
        val updatedContext = context
            ...
            .addStep(
                ProcessingStep(
                    ...
                    output = "Маппинг: ${frameMappings.count { it.node != null }}/${frameMappings.size} фреймов, соседей: ${neighborNodes.size}",
                    stepType = type,
        ...
        log.info(
            ...
            frameMappings.count { it.node != null }, frameMappings.size,
            neighborNodes.size, rootCauseNode?.fqn,
        )
        return StepResult(context = updatedContext, transitionKey = transitionKey)
    }

    private fun findNodeForFrame(appId: Long, frame: StackFrame): Node? {
    ...
    private fun expandRootCause(rootNode: Node): ExpansionResult {
        ...
        val outgoing = edgeRepository.findAllBySrcId(nodeId)
            .filter { it.kind in EXPANSION_EDGE_KINDS }
        val incoming = edgeRepository.findAllByDstId(nodeId)
        ...
        val neighbors = if (neighborIds.isNotEmpty()) {
        ...
        val relationsText = if (allEdges.isNotEmpty()) {
            buildString {
    ...
    private fun buildAnalysisText(
        ...
        return buildString {
            ...
            if (rootCauseNode != null) {
                ...
                appendLine("Класс: ${rootCauseNode.parent?.name ?: rootCauseNode.fqn.substringBeforeLast('.')} (${rootCauseNode.parent?.fqn ?: ""})")
    ...
    private data class FrameMapping(val frame: StackFrame, val node: Node?)
    private data class ExpansionResult(val neighbors: List<Node>, val relationsText: String)
}
