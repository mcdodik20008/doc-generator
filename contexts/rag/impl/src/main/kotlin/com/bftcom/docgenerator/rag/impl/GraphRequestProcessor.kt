...
class GraphRequestProcessor(
    ...
    fun process(
        ...
        while (iterationCount < MAX_ITERATIONS) {
            ...
            if (!visitedSteps.add(currentStep)) {
                log.warn(
                    "Обнаружен цикл в графе обработки запроса: шаг {} уже выполнялся. Путь: {}",
                    currentStep,
                    visitedSteps,
                )
            ...
            if (step == null) {
                log.error(
                    "Не найден обработчик шага {}. Зарегистрированные шаги: {}. Прекращаем обработку",
                    currentStep,
                    registry.keys,
                )
            ...
            if (nextStep == null) {
                log.error(
                    "Не найден переход для ключа '{}' в шаге {}. Доступные переходы: {}",
    ...
    private fun getStepDescription(
        ...
        when (stepType) {
            ...
            ProcessingStepType.INTENT_CLASSIFICATION -> {
                when (status) {
            ...
            ProcessingStepType.HYPOTHESIS_GENERATION -> {
                when (status) {
            ...
            ProcessingStepType.ARCHITECTURE_SYNTHESIS -> {
                when (status) {
            ...
            ProcessingStepType.STACKTRACE_PARSING -> {
                when (status) {
            ...
            ProcessingStepType.STACKTRACE_ANALYSIS -> {
                when (status) {
            ...
            ProcessingStepType.COMPLETED -> {
                "Обработка завершена"
            }

            ProcessingStepType.FAILED -> {
