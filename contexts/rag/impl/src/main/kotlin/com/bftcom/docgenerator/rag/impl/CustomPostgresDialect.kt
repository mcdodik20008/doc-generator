...
class CustomPostgresDialect : PostgresChatMemoryRepositoryDialect() {
    override fun getSelectMessagesSql(): String =
        "SELECT content, type FROM $SCHEMA_NAME.$TABLE_NAME WHERE conversation_id = ? ORDER BY \"timestamp\""

    override fun getInsertMessageSql(): String =
        "INSERT INTO $SCHEMA_NAME.$TABLE_NAME (conversation_id, content, type, \"timestamp\") VALUES (?, ?, ?, ?)"

    override fun getDeleteMessagesSql(): String = "DELETE FROM $SCHEMA_NAME.$TABLE_NAME WHERE conversation_id = ?"
