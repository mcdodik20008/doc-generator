...
class ExactNodeSearchAdvisor(
    ...
    override fun process(context: QueryProcessingContext): Boolean {
        ...
        try {
            ...
            log.debug("Извлечено из запроса: класс={}, метод={}", extractionResult.className, extractionResult.methodName)
            ...
            if (foundNodes.isNotEmpty()) {
                ...
                context.setMetadata(QueryMetadataKeys.EXACT_NODE_SEARCH_RESULT, extractionResult)
                ...
                log.info("Найдено {} узлов для класса '{}' и метода '{}'", foundNodes.size, extractionResult.className, extractionResult.methodName)
            } else {
                log.debug("Узлы не найдены для класса '{}' и метода '{}'", extractionResult.className, extractionResult.methodName)
            }
        } catch (e: Exception) {
            log.warn("Ошибка при точном поиске узлов: {}", e.message, e)
    ...
    private fun extractClassAndMethod(query: String): ExtractionResult? {
        ...
        return try {
            ...
            log.warn("Не удалось распарсить ответ LLM: {}", response, e)
            null
        }
    }
    ...
    private fun findNodes(className: String?, methodName: String?): List<Node> {
        ...
        if (applications.isEmpty()) {
            log.debug("Не найдено приложений для поиска")
            return foundNodes
        }
