...
class Resilience4jConfig {
    ...
    @Bean
    fun llmCircuitBreaker(registry: CircuitBreakerRegistry): CircuitBreaker = registry.circuitBreaker(LLM_CIRCUIT_BREAKER)
...
//            .waitDuration(Duration.ofSeconds(2))
    ...
    @Bean
    fun llmRetry(registry: RetryRegistry): Retry = registry.retry(LLM_RETRY)
    ...
    @Bean
    fun llmBulkhead(registry: BulkheadRegistry): Bulkhead = registry.bulkhead(LLM_BULKHEAD)
}
