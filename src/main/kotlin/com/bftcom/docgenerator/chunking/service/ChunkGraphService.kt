...
import com.bftcom.docgenerator.api.dto.graph.GraphResponse
...
class ChunkGraphService(
    ...
    fun buildGraph(appId: Long, kinds: Set<String>, limit: Int, withRelations: Boolean): GraphResponse {
        val nodes = repo.loadNodes(appId, kinds, limit)
        val edges = repo.loadEdges(appId, nodes.map { it.id }.toSet(), withRelations)
        return GraphResponse(nodes, edges)
    }

    fun expandNode(nodeId: String, limit: Int): GraphResponse {
